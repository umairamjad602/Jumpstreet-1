...
class Model_tables extends CI_Model
{
	...
	public function getTableData($id = null)
	{
		...
		if($user_id == 1) {
			...
			$sql = "SELECT * FROM tables WHERE store_id IN (" . implode(',', json_decode($user_data['store_id'])) . ") ORDER BY id DESC";
			$query = $this->db->query($sql);
			return $query->result_array();
		}
	...
	public function getActiveTable()
	{
		...
		if($user_id == 1) {
			...
			$this->load->model('model_users');
			$user_data = $this->model_users->getUserData($user_id);
			$sql = "SELECT * FROM tables WHERE store_id IN (" . implode(',', json_decode($user_data['store_id'])) . ") AND available = ? AND active = ? ORDER BY id DESC";
			$query = $this->db->query($sql, array(1, 1));
			return $query->result_array();
		}
