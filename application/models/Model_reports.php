...
//    }
    ...
    public function getOrderDataByStartToEnd($start_from, $end_to)
    {
        if($start_from && $end_to) {
            ...
            $daysArray = array();
            foreach ($period as $key => $value) $daysArray [] = $value->format('Y-m-d');

            $order_data = array();
            foreach ($result as $k => $v){
                if(in_array(date('Y-m-d',$v['date_time']),$daysArray)){
    ...
    public function getStoreWiseOrderDataByStartToEnd($start_from, $end_to, $store_id)
    {
        if($start_from && $end_to && $store_id) {

            $sql = "SELECT * FROM orders WHERE paid_status = ? AND store_id = ?";
            $query = $this->db->query($sql, array(1, $store_id));
            $result = $query->result_array();
